...
3. Add the environment variables from the repo root `.env` (or your Supabase project values) in Vercel Project Settings. For a static Vercel deployment, the `VITE_`-prefixed values are required; the non-prefixed equivalents are only needed if you add server/runtime functionality later.
